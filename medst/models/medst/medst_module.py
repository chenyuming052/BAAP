...
class PrintProgressCallback(Callback):
    """Print training progress to stdout for kubectl/k9s logs."""
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, *args):
        if (batch_idx + 1) % self.every_n_steps == 0 or batch_idx == 0:
            metrics = trainer.callback_metrics
            loss = metrics.get("train_loss", 0)
            loss_ita = metrics.get("train_loss_ita", 0)
            loss_local = metrics.get("train_loss_local", 0)
            loss_temp = metrics.get("train_loss_temporal", 0)
            loss_ita_raw = metrics.get("train_loss_ita_raw", loss_ita)
            loss_local_raw = metrics.get("train_loss_local_raw", loss_local)
            loss_temp_raw = metrics.get("train_loss_temporal_raw", loss_temp)
            acc1 = metrics.get("train_acc1", 0)
            lr = trainer.optimizers[0].param_groups[0]["lr"]
            total_steps = trainer.num_training_batches
            pct = (batch_idx + 1) / total_steps * 100 if total_steps else 0
            print(
                f"[Epoch {trainer.current_epoch}] "
                f"Step {batch_idx + 1}/{total_steps} ({pct:.1f}%) | "
                f"loss={float(loss):.4f} "
                f"(w: ita={float(loss_ita):.3f} local={float(loss_local):.3f} "
                f"temp={float(loss_temp):.3f}) "
                f"(raw: ita={float(loss_ita_raw):.3f} local={float(loss_local_raw):.3f} "
                f"temp={float(loss_temp_raw):.3f}) | "
                f"acc1={float(acc1):.1f}% | lr={lr:.2e}",
                flush=True,
            )
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        val_ita = metrics.get("val_loss_ita", 0)
        val_local = metrics.get("val_loss_local", 0)
        val_temp = metrics.get("val_loss_temporal", 0)
        val_ita_raw = metrics.get("val_loss_ita_raw", val_ita)
        val_local_raw = metrics.get("val_loss_local_raw", val_local)
        val_temp_raw = metrics.get("val_loss_temporal_raw", val_temp)
        val_acc1 = metrics.get("val_acc1", 0)
        print(
            f"[Epoch {trainer.current_epoch}] "
            f"===== VAL ===== "
            f"val_loss={float(val_loss):.4f} | "
            f"(w: ita={float(val_ita):.3f} local={float(val_local):.3f} "
            f"temp={float(val_temp):.3f}) "
            f"(raw: ita={float(val_ita_raw):.3f} local={float(val_local_raw):.3f} "
            f"temp={float(val_temp_raw):.3f}) | "
            f"val_acc1={float(val_acc1):.1f}%",
            flush=True,
        )
...
class MedST(LightningModule):
    ...
    def __init__(self,
        ...
        #     nn.Linear(emb_dim * 3, emb_dim * 12),
        #     nn.GELU(),
        #     nn.LayerNorm(emb_dim * 12),
        #     nn.Linear(emb_dim * 12, emb_dim * 3),
        #     nn.GELU(),
    ...
    # def compute_progression_loss(self, img_emb_q, progression_labels):
    #     prog_preds = self.prog_head(img_emb_q)
    #     loss = F.cross_entropy(prog_preds, progression_labels)
    #     return loss
    ...
    def forward(self, batch, batch_idx, split="train"):
        ...
        device = loss_ita.device


        ################  temporal_modeling  ########
        loss_temporal = torch.zeros((), device=device)
        if self.hparams.lambda_3 > 0 and int(s_len / 4) != 0:
            s_shape = (int(s_len / 4), 4, -1)
            loss_series1 = self.comput_alignment_loss(
                img_emb_q[:s_len, :].reshape(s_shape),
                report_emb_q[:s_len, :].reshape(s_shape),
                huber_delta=self.hparams.huber_delta,
            )
            loss_series2 = self.comput_alignment_loss(
                report_emb_q[:s_len, :].reshape(s_shape),
                img_emb_q[:s_len, :].reshape(s_shape),
                huber_delta=self.hparams.huber_delta,
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        weighted_ita = self.hparams.lambda_1 * loss_ita
        weighted_local = self.hparams.lambda_2 * loss_local
        weighted_temporal = self.hparams.lambda_3 * loss_temporal
        loss = weighted_ita + weighted_local + weighted_temporal

        log = {
            "train_loss": loss,
            "train_loss_ita": weighted_ita,
            "train_loss_local": weighted_local,
            "train_loss_temporal": weighted_temporal,
            "train_loss_ita_raw": loss_ita.detach(),
            "train_loss_local_raw": loss_local.detach(),
            "train_loss_temporal_raw": loss_temporal.detach(),
            "train_acc1": acc1,
            "train_acc5": acc5
        }
    ...
    def validation_step(self, batch, batch_idx):
        ...
        log = {
            "val_loss": loss,
            "val_loss_ita": weighted_ita,
            "val_loss_local": weighted_local,
            "val_loss_temporal": weighted_temporal,
            "val_loss_ita_raw": loss_ita.detach(),
            "val_loss_local_raw": loss_local.detach(),
            "val_loss_temporal_raw": loss_temporal.detach(),
            "val_acc1": acc1,
            "val_acc5": acc5
        }
        self.log_dict(log, batch_size=self.hparams.batch_size,
    ...
    def precision_at_k(output: torch.Tensor, target: torch.Tensor, top_k=(1,)):
        ...
        with torch.no_grad():
            ...
            for k in top_k:
                actual_k = min(k, maxk)
                correct_k = correct[:actual_k].contiguous(
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="Optional checkpoint used to initialize overlapping model weights.",
        )
...
def cli_main():
    ...
    parser.set_defaults(max_epochs=8)
    ...
    print("disable_early_stopping: ", args.disable_early_stopping)
    print("----------------")
    ...
    if args.pretrained_ckpt:
        ...
        old_model_state_dict = torch.load(args.pretrained_ckpt, map_location="cpu")
        if "state_dict" in old_model_state_dict:
            old_model_state_dict = old_model_state_dict["state_dict"]
        old_model_state_dict = {
            k: v for k, v in old_model_state_dict.items() if k in new_model_state_dict
        }
    ...
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../experiments/results/{exp_name}/{run_tag}")
    extension = f"{exp_name}_{run_tag}"
    os.makedirs(ckpt_dir, exist_ok=True)
    callbacks = [
        LearningRateMonitor(logging_interval="step"),
        ModelCheckpoint(monitor="val_loss", dirpath=ckpt_dir,
                        save_last=True, mode="min", save_top_k=5),
        PrintProgressCallback(every_n_steps=50),
    ]
    ...
    wandb_logger = WandbLogger(
        project="MedST", save_dir=ckpt_dir, name=extension)
