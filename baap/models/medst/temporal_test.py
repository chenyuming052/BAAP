...
DEFAULT_DATA_ROOT = Path(os.environ.get("BAAP_DATA_DIR", PROJECT_ROOT / "data"))
MSCXRT_IMAGE_CSV = DEFAULT_DATA_ROOT / "ms-cxr-t" / "MS_CXR_T_temporal_image_classification_v1.0.0.csv"
MSCXRT_TEXT_CSV = DEFAULT_DATA_ROOT / "ms-cxr-t" / "MS_CXR_T_temporal_sentence_similarity_v1.0.0.csv"
MIMIC_CXR_FILES_DIR = DEFAULT_DATA_ROOT / "mimic-cxr-jpg-2.1.0" / "files"
...
class MedST_temporal(LightningModule):
    def __init__(self,
        ...
        self.model = MedST.load_from_checkpoint(path, strict=False).to(self.device_type)
    ...
    def get_embeddings_from_prompt(self,prompt):
        ...
        cap_id = tokens["input_ids"].to(self.device_type)
        attention_mask = tokens["attention_mask"].to(self.device_type)
        token_type_ids = tokens["token_type_ids"].to(self.device_type)
    ...
    def get_image_feat(self, path_list):
        ...
        for i in path_list:
            ...
            # print(sizee)
        with torch.no_grad():
            f, p = self.img_encoder_q(torch.stack(imgs).to(self.device_type))
