...
class MultimodalPretrainingDataset(data.Dataset):
    def __init__(self, split="train", transform=None, data_pct=1.0,
        ...
        if data_pct != 1.0 and split == "train":
            np.random.seed(42)
            n = max(1, int(len(self.temporal) * data_pct))
            idx = np.random.choice(len(self.temporal), size=n, replace=False)
            self.temporal = [self.temporal[i] for i in idx]
        ...
        with FileLock(lock_path):
            if not os.path.isfile(filepath):
                ...
                try:
                    ...
                    os.unlink(tmp)
                    raise
                fd1, tmp1 = tempfile.mkstemp(dir=cache_dir, suffix=".tmp")
                try:
                    with os.fdopen(fd1, "wb") as f:
                        pickle.dump(zheng2ce, f, protocol=2)
                    os.replace(tmp1, filepath1)
                    print("Save to: ", filepath1)
                except BaseException:
                    os.unlink(tmp1)
                    raise

        # Read outside the lock — only if this process didn't create them
        if path2sent is None:
...
def multimodal_collate_fn(batch):
    # imgs1: images in seq (length-4 temporal sequences)
    # imgs2: images not in seq (length != 4)
    imgs1, imgs2 = [], []
    ces1, ces2 = [], []
    lateral_idxs1, lateral_idxs2 = [], []
    path = []
    ...
    for b in batch:
        ...
        if(len(imgs) == 4):
            imgs1.extend(imgs)
            ces1.extend(ce_list)
            lateral_idxs1.extend(lateral_idx)
    ...
    ces1.extend(ces2)
    lateral_idxs1.extend(lateral_idxs2)
    ...
    lateral = lateral_idxs1
    ces = torch.stack(ces1)
